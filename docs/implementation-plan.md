...
3. 预约业务类型：以 `需求` TAB 为准，包含供应商到货、调配出库、调配入库、施工领料。系统管理员可新增、删除、启用、禁用。
4. 号段类别：根据用户访谈补充为供应商、承运商、领料单位三类。号段类别是容量管理维度，不替代四种预约业务类型；两者通过适用关系关联。
5. 发起方角色：支持施工队、供应商、承运商以及仓库管理员提交预约信息。账号/单位类型用于身份与考核，号段类别用于容量隔离。
6. 审核链路：放号审核和同日履约调整由中心管理员处理；预约审核由仓库管理员处理。审批链分开记录。
7. 禁约名单：MVP 不单独实现“禁约名单管理”页面。取消或未到现场次数达到阈值后，通过资格校验禁止当月预约；如后续需要独立名单，推荐按账号维度落地，并关联单位用于统计。
8. 临时预约：以需求为准，仓库临时创建号段并注明原因，中心站管理员审核通过后，用户可办理临时预约。履约调整使用独立调整单，不与临时号段混用。
9. 履约闭环：每笔审核通过的预约必须形成正常闭环、异常闭环或未到现场闭环。每日定时任务关闭未处理流程时，履约结果必须标记为“未到现场”，不能计入正常完成。
10. 仓库层级：号段直接归属仓库，不再设置仓库下的二级作业区域及相关配置、选择和校验。
...
|---|---|---|---|---|---|---|
| U01 | 可预约号段周日历 / 预约首页 | PC + 移动 | 施工队、供应商、承运商 | 以周日历展示每天开放时段、剩余容量和已满状态；点击具体时间直接开始预约；只能本周预约下周号段 | `预约/08_appt_booking.html` | 必做 |
| U02 | 新建预约 | PC + 移动 | 施工队、供应商、承运商 | 在周日历中可视化选择日期和时间；按号段自动匹配可办理业务；默认带出并可修改单位、联系人和联系方式；提交预约 | `预约/08_appt_booking.html` | 必做 |
| U03 | 我的预约列表 | PC + 移动 | 预约发起人 | 查看本人/本单位预约记录、审核状态、完结状态、取消状态 | `预约/08_appt_booking.html`、`预约/08a_booking_detail.html` | 必做 |
| U04 | 预约详情 | PC + 移动 | 预约发起人 | 查看预约编号、类型、仓库、号段、审核结果、操作历史 | `预约/08a_booking_detail.html` | 必做 |
...
| M11 | 新增/编辑号段 | 仓库管理员 | 按仓库和一小时号段维护日期、时间、三类固定配额或共享总容量；提交前可改可删 | `预约/07a_slot_form.html` | 必做 |
...
| M18 | 预约审核详情 | 仓库管理员 | 查看单位、联系人、联系方式、预约类型、仓库、号段；通过/驳回 | `预约/09a_review_detail.html` | 必做 |
| M19 | 预约驳回原因 | 仓库管理员 | 填写驳回原因并通知预约联系人 | `预约/09b_reject_reason.html` | 必做，可用弹窗 |
| M20 | 履约闭环工作台 | 仓库管理员、中心管理员 | 分区处理待履约、原号段未到、调整待审批、调整后到场、已闭环预约 | `预约/09c_complete_confirm.html` | 必做，按访谈补齐闭环流程 |
| M20A | 同日履约调整 | 仓库管理员 | 原号段未到后选择同仓、同日、后续同类别号段，填写原因并提交审批 | 原型未覆盖 | 必做 |
| M20B | 履约调整审批 | 中心管理员 | 审批调整申请；通过时重新校验并占用目标号段容量 | 原型未覆盖 | 必做 |
...
| 号段周日历/时间选择器 | U01、U02、U07 | 固定展示周一至周日，区分未开放、可预约、余量紧张和已满；点击日期后直接选择具体时间 |
| 号段容量编辑器 | M11、M13 | 同一时间段维护供应商、承运商、领料单位三类固定配额，或设置三类共享总容量 |
| 预约类型选择器 | U02、M28 | 读取业务类型配置，默认包含四类，并按所选号段类别校验适用关系 |
...
| 履约闭环面板 | M20 | 区分正常闭环、异常闭环、未到现场和未闭环，展示原始及调整后号段 |
| 同日调整申请卡 | M20A | 原号段未到后选择后续号段、填写原因并提交中心审批 |
| 履约调整审批表 | M20B | 展示原号段、目标号段、类别和剩余容量，支持通过或驳回 |
| 临时号段创建表单 | M21 | 仓库管理员填写仓库、日期、时间、三类容量和临时原因 |
...
4. 号段容量按供应商、承运商、领料单位三类记录配置数、已预约数和剩余数。固定配额互不占用；共享模式共同占用总容量。

### 3.2 预约状态机

预约由用户前端或仓库管理员提交，仓库管理员审核。履约结果与流程关闭状态分别记录，避免“自动关闭”被误计为正常完成。
...
stateDiagram-v2
  ...
  PendingCompletion --> Completed: 原号段到场，正常闭环
  PendingCompletion --> MissedWindow: 原号段结束仍未到
  MissedWindow --> AdjustmentPending: 仓库发起同日调整
  AdjustmentPending --> Rescheduled: 中心审批通过
  AdjustmentPending --> MissedWindow: 中心审批驳回
  Rescheduled --> Completed: 调整后到场，异常闭环
  PendingCompletion --> NoShow: 人工确认未到现场
  MissedWindow --> NoShow: 人工确认未到现场
  Rescheduled --> NoShow: 调整后仍未到现场
  PendingCompletion --> AutoNoShow: 次日00:00自动闭环
  MissedWindow --> AutoNoShow: 次日00:00自动闭环
  AdjustmentPending --> AutoNoShow: 次日00:00自动闭环
  Rescheduled --> AutoNoShow: 次日00:00自动闭环
  Rejected --> [*]
  Cancelled --> [*]
  Completed --> [*]
  AutoNoShow --> [*]
...
5. 原号段未到不是终态，可发起同仓、同日、后续同类别号段调整，且必须经过中心管理员审批。
6. 正常闭环记录 `arrivalStatus=onTime`；异常闭环记录 `arrivalStatus=rescheduledArrived`；未到闭环记录 `arrivalStatus=notArrived`。
7. 每日定时任务只关闭流程，自动闭环的履约结果必须是“未到现场”，并累计过号次数、发送通知。
8. 定时任务必须幂等，重复执行不得重复累计过号。
...
| 号段容量 | 保存三类固定配额或共享总容量、各类已占用和剩余容量 | 固定配额互不占用，共享模式共同扣减 |
...
| 履约处理 | 标记正常办结、原号段未到、调整后办结或未到现场 | 同时保存流程状态、到场结果和闭环类型 |
| 履约调整 | 查询可调整目标号段、发起、审批通过、驳回 | 仅同仓同日后续同类别号段；审批通过时原子校验容量 |
| 未到处理 | 人工或系统关闭、累计次数、通知联系人 | 达阈值后当月禁约 |
| 自动闭环任务 | 每日 00:00 扫描仍未闭环预约并标记未到现场 | 需要幂等、任务日志，不能计入正常完成 |
...
| 统计 | 月/年统计、单位详情、仓库统计、中心站统计、趋势分析 | 指标包含正常闭环、异常闭环、未到现场、未闭环、取消和临时预约 |
...
| AC-05 | 号段仓库归属 | 仓库管理员登录 | 新增普通号段或临时号段 | 号段直接归属所选仓库；无需选择额外的仓库下级区域 |
| AC-06 | 创建普通号段 | 仓库管理员登录 | 新增下周一小时号段并配置三类容量 | 号段保存为待提交；三类容量字段完整；提交前可修改和删除 |
| AC-07 | 共享容量模式 | 仓库管理员创建号段 | 设置三类共享总容量 | 各类预约共同扣减总容量，超过总容量不可提交 |
...
| AC-13 | 用户查看可预约号段 | 用户本月未被禁约 | 在 PC 和移动端进入预约首页 | 以完整周日历展示下周；每天可直观看到开放时段数，具体时间显示已预约数和剩余数；已满时段不可选择 |
...
| AC-21 | 正常履约闭环 | 已通过预约在原号段到场 | 仓库管理员标记正常办结 | 流程关闭；到场结果为按时到场；闭环类型为正常闭环 |
| AC-22 | 原号段未到 | 已通过预约在原号段结束时仍未到场 | 仓库管理员标记原号段未到 | 预约进入异常处置但尚未闭环，可发起同日调整或按未到现场关闭 |
| AC-23 | 过号阈值禁约 | 用户当月过号次数达到阈值 | 用户尝试新增预约 | 系统阻止预约并提示当月禁止预约 |
| AC-24 | 自动未到闭环 | 当天存在仍未处理的履约预约 | 次日 00:00 后查看预约 | 系统关闭流程并标记未到现场，累计过号并记录任务日志；不计入正常完成，重复执行不重复累计 |
...
| AC-28 | 用户端响应式适配 | 使用 PC 宽屏和 390px 移动端宽度访问用户前端 | 查看 U01-U07 页面并完成预约主流程 | 七天日历完整可见、无横向溢出，具体时段和底部提交按钮可触达，PC 和移动端业务结果一致 |
...
| AC-32 | 三类固定配额隔离 | 同一号段供应商3号、承运商2号、领料单位1号 | 供应商号全部约满后继续预约 | 供应商预约被拦截，承运商和领料单位剩余容量不受影响 |
| AC-33 | 三类共享容量 | 号段设置共享总容量6 | 三类单位依次预约 | 所有类别共同扣减总容量，第7笔预约被拦截 |
| AC-34 | 发起同日履约调整 | 预约原号段未到，且同仓同日存在后续同类别号段 | 仓库管理员选择后续号段并提交 | 生成独立调整单，保留原预约时间，预约进入调整待审批 |
| AC-35 | 调整审批容量校验 | 调整单待审批 | 中心管理员点击通过 | 系统再次校验目标容量；有余量则占用并进入等待调整后到场，无余量则拒绝操作 |
| AC-36 | 异常履约闭环 | 调整审批通过且用户在调整号段到场 | 仓库管理员标记调整后办结 | 流程关闭；到场结果为调整后到场；闭环类型为异常闭环 |
| AC-37 | 闭环统计守恒 | 存在各种履约结果 | 查看统计 | 正常闭环＋异常闭环＋未到现场＋未闭环等于有效预约总数 |

## 6. 执行顺序建议

1. 第一阶段：基础配置和权限。完成中心站、仓库、管理员绑定、预约类型、阈值配置。
2. 第二阶段：放号闭环。完成普通号段创建、容量、提交、中心审核、可预约号段查询、复制下周、截止时间规则。
3. 第三阶段：预约闭环。完成用户端 PC + 移动端预约、我的预约、详情、取消、仓库预约审核。
4. 第四阶段：履约和管控。完成正常闭环、同日调整审批、异常闭环、未到现场、自动闭环、阈值禁约和消息通知。
...
| 问题 | 当前建议 |
|---|---|
| 三类号段与四类业务类型如何映射 | 作为可配置适用关系：供应商到货可使用供应商/承运商号；调配出入库使用承运商号；施工领料使用领料单位号 |
| “预约成功”通知发生在提交后还是审核通过后 | 建议提交后通知“提交成功”，审核通过后通知“预约审核通过” |
| “前一天取消”的截止时间 | 建议按自然日判断：预约日期前一日 23:59:59 前允许取消；如要更严格可配置 |
| “过号”与“原号段未到”术语 | 原号段未到是可调整的处理中状态；当日最终未到现场才形成过号考核记录 |
