...
| M11 | 新增/编辑号段 | 仓库管理员 | 按一小时号段维护日期、时间、作业面、三类固定配额或共享总容量；提交前可改可删 | `预约/07a_slot_form.html` | 必做 |
...
| 临时号段创建表单 | M21 | 仓库管理员填写日期、时间、作业面、三类容量和临时原因 |
...
| AC-06 | 创建普通号段 | 仓库管理员登录 | 新增下周一小时号段，选择作业面并配置三类容量 | 号段保存为待提交；三类容量字段完整；提交前可修改和删除 |
